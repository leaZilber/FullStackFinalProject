...
const SchedulePage = () => {
  ...
  const loadDoctorTurns = async (doctorCode: number) => {
    try {
      const response = await fetch(`https://localhost:7245/api/Doctor/${doctorCode}`);
      const data = await response.json();
      setSelectedDoctor({ ...data, DoctorSchedule: { Turns: data.DoctorSchedule.Turns.filter((t: any) => !t.ConfirmationStatus) } });
    } catch (error) {
  ...
  const registerTurn = async (turn: any) => {
    try {
      const response = await fetch(`https://localhost:7245/api/Doctor/${selectedDoctor?.DoctorCode}`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        loadDoctorTurns(selectedDoctor?.DoctorCode!); // רענון התורים
      } else {
            ...
            <table>
              ...
              <tbody>
                {selectedDoctor.DoctorSchedule.Turns.map((turn: any) => (
                  <tr key={turn.TurnId}>
                    <td>{new Date(turn.DateTurn).toLocaleDateString()} {turn.Hour}</td>
                    <td>{turn.TurnLocate}</td>
                    <td>
                      <button onClick={() => registerTurn(turn)}>קבע תור</button>
                    </td>
                    <td>
                      <button onClick={() => setSelectedTurn(turn)}>פרטים</button>
                    </td>
